...
pipeline {
    ...
    stages {
                ...
                script {                    
                    ...
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            if (!openshift.selector("template/naming-service-buildconfig").exists()) {
                            ...
                            def model = openshift.process('naming-service-buildconfig',
                                    "-p", 
        ...
        stage("Build Container Images") {
            parallel {
                stage("Naming Service") {
                    steps {
                        echo "Building Thoth Naming Service container image..."
                        script {
                            tagMap['naming-service'] = aIStacksPipelineUtils.buildImageWithTag(CI_TEST_NAMESPACE, "naming-service", "${env.TAG}")
                        }
        ...
        stage("Deploy to Test") {
            steps {
                script {
                    // aIStacksPipelineUtils.redeployFromImageStreamTag(CI_TEST_NAMESPACE, "naming-service", "${env.TAG}")
                    // redeploy from ImageStreamTag ${env.TAG}
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            echo "Creating test tag from naming-service:${env.TAG}"

                            openshift.tag("${CI_TEST_NAMESPACE}/naming-service:${env.TAG}", "${CI_TEST_NAMESPACE}/naming-service:test")
                            ...
                            deploymentConfig = openshift.selector("dc", "naming-service")
        ...
        stage("Tag stable image") {
            steps {
                script {
                    ...
                    openshift.withCluster() {
                        openshift.withProject(CI_TEST_NAMESPACE) {
                            echo "Creating stable tag from naming-service:${env.TAG}"

                            openshift.tag("${CI_TEST_NAMESPACE}/naming-service:${env.TAG}", "${CI_TEST_NAMESPACE}/naming-service:stable")
                        }
                    } // withCluster
                } // script
            } // steps
